    ...
    <div>
         <el-form label-width="100px" class="form" v-show="customReg">
             ...
             <el-form-item label="学生姓名">
                <el-input v-model="name" placeholder="请输入学生姓名" clearable class="formInput"></el-input>
            </el-form-item>


            <el-form-item>
                <el-button class="formButton" @click="submit" icon="el-icon-check" type="primary">注册</el-button>
                <el-button class="formButton" @click="CustomRegister()" icon="el-icon-close">取消</el-button>
            </el-form-item>

        </el-form>

        <el-button @click="randomInitData(5)" class="toolBarButton" icon="el-icon-question">新增随机学生</el-button>
        <el-button @click="CustomRegister()" class="toolBarButton" icon="el-icon-info">新增自定义学生</el-button>
...
export default {
    ...
    methods:{
        ...
        randomInitPassword(seed,size){
            const passwordSeed= ['A','B','C','D','E','F','G','H','I','J','K','L','M','N','P','Q','R',
            'S','T','U','V','W','X','Y','Z','a','b','c','d','e','f','g','h','i','j','k','m','n','p',    
            'Q','r','s','t','u','v','w','x','y','z','1','2','3','4','5','6','7','8','9']
            let password=''
            for(let i=0;i<size;i++){
                password+=passwordSeed[Math.round(Math.random(seed)*(passwordSeed.length-1))]
            }
        ...
        //随机生成一个名字
        randomInitName(seed){
            const familyNameSeed=['前','湛','夕','南','东','方','戏','曲','言','墨','花','居','多','巧','贸','声','波']
            const givenNameSeed=['依','白','暄','和','向','雪','俊','美','从','筠','香','薇','线','翰','海','腾','云',
            '霞','蓓','蕾','天','青','正','娴','婉','以','珊','慕','傲','双']
            let fullname =''
            if(Math.floor(Math.round(Math.random(seed)))==1){
                fullname = familyNameSeed[Math.round(Math.random(seed)*(familyNameSeed.length-1))] 
                + givenNameSeed[Math.round(Math.random(seed)*(givenNameSeed.length-1))] 
                + givenNameSeed[Math.round(Math.random(seed)*(givenNameSeed.length-1))]
            }else{
                fullname = familyNameSeed[Math.round(Math.random(seed)*(familyNameSeed.length-1))] 
                + givenNameSeed[Math.round(Math.random(seed)*(givenNameSeed.length-1))]
